...
const setupWS = provider => {
  if (provider.shouldConnect && provider.ws === null) {
    const websocket = new CustomWebSocket(provider.url, provider.connection, provider.roomname)
    ...
    websocket.onmessage = (event: MessageEvent) => {
      console.log(`in: ${event.data.byteLength}`)
      console.log(new TextDecoder().decode(new Uint8Array(event.data)))

      const fromSignalR = (event as any).fromSignalR
      ...
      if (text.trim() && !text.includes('unavatar')) {
        // console.log(`in ${fromSignalR ? 'signal' : 'ws'}: ` + text)
      }

      provider.wsLastMessageReceived = time.getUnixTime()
      const encoder = readMessage(provider, new Uint8Array(event.data), true)
      if (encoding.length(encoder) > 1) {
        websocket.sendStuff(encoding.toUint8Array(encoder))
      }
    }
      ...
      if (provider.wsconnected) {
        ...
        awarenessProtocol.removeAwarenessStates(
          ...
          Array.from(provider.awareness.getStates().keys() as number[]).filter(
            (client: any) => client !== provider.doc.clientID,
          ),
          provider,
        )
    ...
    websocket.onopen = n => {
      ...
      if (provider.awareness.getLocalState() !== null) {
        ...
        websocket.sendStuff(encoding.toUint8Array(encoderAwarenessState))
      }
    }
    provider.emit('status', [
      {
...
class CustomWebSocket extends WebSocket {
  ...
  constructor(url, connection, roomname) {
    super(url)

    this.wsEnabled = false
    this.signalEnabled = true

    // fetch(`https://localhost:5000/mode`).then(val => {
    //   val.text().then((txt: string) => {
    //     if (txt.includes('WebSocket')) {
    //       this.wsEnabled = true
    //     }
    //
    //     if (txt.includes('SignalR')) {
    //       this.signalEnabled = true
    //     }
    //   })
    // })
    ...
    this.connection.on('Outgoing', data => {
      var arr: number[] = JSON.parse(data)
      var buff: ArrayBuffer = new Uint8Array(arr).buffer
  ...
  sendStuff(data) {
    if (this.signalEnabled) {
      if (this.connection.state === signalR.HubConnectionState.Connected) {
        ...
        this.connection.send('Incoming', JSON.stringify(Array.from(data)))
      } else {
 ...
 */
export class WebsocketSignalProvider extends Observable<string> {
    ...
    if (resyncInterval > 0) {
      this._resyncInterval = /** @type {any} */ setInterval(() => {
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          ...
          this.ws.sendStuff(encoding.toUint8Array(encoder))
        }
      }, resyncInterval)
    }
